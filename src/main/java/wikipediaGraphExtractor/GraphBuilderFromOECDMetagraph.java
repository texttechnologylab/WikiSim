...
public class GraphBuilderFromOECDMetagraph {
	...
	public static void main(String[] args) throws JSONException, IOException, SQLException {
		...
		for (Object object : json) {
			...
			System.out.println(thema.toString(4));

			if(true)
				continue;
			
			
			Path fullgml = Paths.get("graphs/oecdTopics/","fullgml",thema.getString("name"));
			fullgml.toFile().mkdirs();

			Path gmlPath = Paths.get("graphs/oecdTopics/","gml",thema.getString("name"));
			...
			languageWikidataMap.entrySet().forEach(
					entry->{
						...
						entry.getValue().forEach(
								link -> {
									System.out.println(language + "\t"+thema.getString("name")+"\t"+current++ + "/" + entry.getValue().size());
